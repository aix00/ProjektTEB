...
public class AccountRepository {
    ...
    public long createAccount(AccountEntity account){
        AccountEntity accountEntity = new AccountEntity(account.getUserName(), account.getPassword(),account.getEmail(), account.getBirthDate(), account.getFavouriteThing(), account.getSex(), account.isAdmin());
    ...
    public Collection<AccountEntity> getAccounts(){
        ...
        for (AccountEntity accountEntity : accountDatabaseRepository.findAll()){
            account.add(new AccountEntity(accountEntity.getUserName(), accountEntity.getPassword(),accountEntity.getEmail(), accountEntity.getBirthDate(), accountEntity.getFavouriteThing(), accountEntity.getSex(), accountEntity.isAdmin()));
        }
        return account;
    }
    ...
    public AccountEntity getAccount(long id) throws NotFoundException{
        ...
        return new AccountEntity(accountEntity.getUserName(), accountEntity.getPassword(), accountEntity.getEmail(), accountEntity.getBirthDate(), accountEntity.getFavouriteThing(), accountEntity.getSex(), accountEntity.isAdmin());
    }
}
